...
- Gives every row a clickable Publish checkbox. Uncheck a row to leave it out of the next batch.
...
- Optionally clears existing renderer releases `v1` to `v10`.
- Clears complete Render Release Zips workflow jobs and their attached artifacts without deleting local zips or release assets.
...
- Copies download links for all highlighted completed rows.
- Downloads all highlighted completed jobs to a persistent chosen folder, extracts their MP4 files, and deletes the downloaded artifact ZIP files automatically.
